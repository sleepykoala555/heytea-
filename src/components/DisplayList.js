      ...
      {filteredList.map((item, index) => ( // TODO: map bakeryData to BakeryItem components
        <Card style={{ width: '20rem' }} className="card">
          ...
          <Card.Body>
            ...
            <p>  <button className="button-24" role="button" onClick={() => props.addToCart(item)}>ADD TO CART</button> </p>
            <p>  <button className="button-25" role="button" onClick={() => props.removeCart(item)}>REMOVE</button> </p>
          </Card.Body>
        </Card>
      ))}
    </div>
